...
class HistoryBill(Window):
    ...
    def __init__(selfBill,key):
        ...
        for filename in files:
            ...
            try:
                newBtn = Button()
                newBtn.Content = "{} : {}".format(invoice_no,str(cust_name).strip())
                newBtn.Name = str('Cust_'+str(selfBill.tot_cust))
                selfBill.cust_files[int(selfBill.tot_cust)] = {'c_month':data_dict['c_month'],'c_year':data_dict['c_year'],'filename': filename,'invoice_no':invoice_no, 'cust_name': str(cust_name), 'grand_total': data_dict['grand_total']}
                if "grand_total" in data_dict.keys():
                    newBtn.ToolTip = "Total: {}".format(str(data_dict['grand_total']))
                else:
                    newBtn.ToolTip = ""
                newBtn.Height = 0
                newBtn.Width = 360
    ...
    def sortByDate(self,sender,e):
        ...
        for challan in self.cust_files:
            ...
            try:
                newBtn = Button()
                newBtn.Content = "{} : {}".format(str(challan['invoice_no']),str(challan['cust_name']).strip())
