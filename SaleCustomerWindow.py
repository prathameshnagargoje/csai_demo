...
class SaleCustomerWindow(Window):
    key = ""
    custDict = {'tot_rows':0}
    data_cc=0
    current_data_no=0
    ...
    def __init__(selfCust,key,filename=""):
        wpf.LoadComponent(selfCust, 'SaleCustomerWindow.xaml')
        selfCust.key = key
        ...
        if id(str(filename).strip())!=id(""):
            data_dict = {}
            with open("sale_cust\\{}".format(str(filename))) as fp:
    ...
    def saveFinalData(self,sender,e):
        try:
            ...
            file_path = "sale_cust/"+filename+".csai"
            if not os.path.isdir('sale_cust'):
                os.makedirs('sale_cust')
            with open(file_path, 'w') as fp:
                json.dump(data, fp)
            sync_data = "sale_cust\t"+filename+"\n"
            with open("sync_list.csai",'a') as fp:
                fp.write(sync_data)
            MessageBox.Show("saleCustomer Data has been saved...")
            self.Close()
        except:
    ...
    def genPDF_Click(self,sender,e):
        try:
            ...
            loadingWindow.Show()
            self.Hide()
            args = ['ppython/python.exe', 'customerPDF.py', 'sale_cust/{}'.format(filename)]
    ...
    def deleteCustomer_Click(self,sender,e):
        try:
            filename = self.custDict['filename'].split(".csai")[0]
            if filename != "":
                file_path = "sale_cust/"+filename+".csai"
                try:
                    os.remove(file_path)
                    with open("sync_list.csai",'a')as fp:
                        fp.write("del\tsale_cust\t{}\n".format(filename))
                except:
                    pass
            self.Close()
