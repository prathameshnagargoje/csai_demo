...
from ServiceCustomer import ServiceCustomer

class HistoryServiceCustomer(Window):
    key = ""
    tot_cust = 0
    total_rembal=0
    cust_files = {}
    ...
    def __init__(hscSelf,key):
        hscSelf.key = key
        wpf.LoadComponent(hscSelf, 'HistoryServiceCustomer.xaml')
        hscSelf.tot_cust = 0
        files = os.listdir("service_cust")
        ...
        for filename in files:
            ...
            path = "{}\\{}".format("service_cust",str(filename))
    ...
    def custBtn_Click(self,sender,e):
        ...
        if cust_no in self.cust_files.keys():
            ...
            form = ServiceCustomer(self.key,filename)
            form.Show()
            self.Close()
